...
$query = mysqli_query($conn, "SELECT * FROM soal WHERE mode='php' AND aktif='Y'");
...
if (isset($_POST["submit"])) {
    ...
    for ($i = 0; $i < $jumlah; $i++) {
        ...
        if (empty($pilihan[$nomor])) {
            ...
            $sql = "SELECT * FROM soal WHERE id='$nomor' AND mode='php' AND jawaban='$jawaban'";
        ...
        $sql = "SELECT * FROM soal WHERE mode='php' AND aktif='Y'";
        $query = mysqli_query($conn, $sql) or die(mysqli_error($conn));
    ...
    header("location:../Result.php?php");
}
